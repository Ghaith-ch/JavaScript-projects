...
async function getRandomWord() {
  ...
  const categories = data.map(categoryData => categoryData.category);
  const selectedCategory = categories[Math.floor(Math.random() * categories.length)];

  let categoryInfo = document.querySelector(".category span");
  categoryInfo.innerHTML = selectedCategory;

  const selectedCategoryData = data.find(categoryData => categoryData.category === selectedCategory);
  const selectedCategoryWords = selectedCategoryData.words;
